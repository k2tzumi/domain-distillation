...
<!--
まず、私たちが扱っている「レセプト業務」がどれだけ複雑か、視覚的にお見せします。これがその業務のルールブック、「事業者ハンドブック」です。  
皆さんも一家に一冊お持ちであろう「ちょうぜつ本」と比較しても、この圧倒的な厚さです。
-->
...
先ほど見て頂いた分厚い本の大半がこのルールの原文および解釈に当たられています。
-->
...
<!--
この算定構造の作り方をテーマにした発表をPHPカンファレンス関西で行いましたので、よかったらご参照ください
-->
...
<!--
さらに、プロセス特有の構造上の問題があります。  
業務フロー上、各種の実績データ、つまり上流のデータを掻き集めてきて、それを基にレセプト、つまり下流でガラガラ計算して請求データを作ります。  
この「データの流れ」が、そのままシステムの「依存の方向」になっていました。
-->
...
<!--
ここに大きな「矛盾」があります。　　
レセプト業務は、障害福祉の事業所を運営する為に必要不可欠な業務であり、「最も複雑」でミスの影響が大きく、「最も安定させるべき」ものです。    
[click] 「なのに」、データの流れの「下流」に位置してしまっています。  
[click] 一方、上流の実績データは、使いやすさを追求するため、「仕様確定に時間がかかり」、「変更が頻繁」に発生します。  
[click] パターンも多く検証に時間もかかります。  
[click] 結果、最も安定すべきレセプト業務が、上流の変更の影響を直接受け、「最も不安定な位置」に置かざるをえないというのが辛みとしてありました。
-->
...
<!--
法改正の難しさの解像度が上がりましたでしょうか？  
[click] この状況で、1チームで完結しない大規模な法改正に対応を行う必要があります。  
[click] 上流の仕様FIXを待ってから実装していたら、到底間に合わなくなります。
-->
...
<!--
法律の施行日は国によって定められており、システム開発側の都合で延期することは絶対にできず、どうすれば。。と頭を抱えることになります。。
-->
...
2つ目は、法改正の影響がない請求状態の管理など、周辺の関心事を外に切り出しました。  　　
[click] 取り組みの詳細内容は記事がありますので、ご参照ください。
-->
...
<!--
この「関心事の分離」によるメリットは3つありました。  
[click] まず、実装効率の向上です。旧制度のコードに触れず、新制度のロジック追加に集中できました。  
[click] 次に、テスト範囲が限定的にできました。新規ロジックのみをテストすればよくなりました。  
[click] そして、保守性の向上です。役割を終えた旧制度のロジックを安全に削除でき、システムの複雑化を防げます。　　
[click] モジュールとして独立させやすくなりました  
責務が明確になって、サービスとして切り出しやすくなりました  
あくまで印象です。実際に切り出すには、まだまだ足りない部分がありましたがw
-->
...
<!--
法改正を乗り越えたら、今度はビジネスが拡大しました。  
[click] プロダクトが増加し、関係者がシステム間で分散・重複しはじめ、システム「全体」の複雑さが急上昇しました。
-->
...
1 つのサービス的には改善されましたが、**全社視点**では限界が見えました。
...
<!--
1つのサービスの「個別最適」では限界が見えました。  
全社視点で法改正を乗り切るため、次のレベルの構造化が必要になりました。
-->
...
<!--
具体的には、レセプト業務をBaaS化する、「Rezept as a Service」です。
-->
...
[click]「でも、依存の方向は変えられる！」  
[click] 実績データが流れるという「データの流れ（事実）」と、[click] 設計上「どちらがどちらに依存するか（設計）」は、[click] イコールである必要はない、と考えです。
-->
...
<!--
これが、Rezept as a Serviceの概念図です。レセプト業務、つまり算定ロジックを「上位モジュール」として中央に置きます。そして、「APIインターフェース」という「契約定義」を定めます。  
各プロダクト、つまり下位モジュールは、この「契約」に従って実装します。  
最も重要な点として、上位モジュールである「算定ロジック」もまた、自らが定義した「APIインターフェース」にのみ依存します。これにより、依存関係が逆転しました。
-->
...
[click] Afterの状態では、レセプト業務がコア業務としてサービス化され、他に依存しない「安定」した状態になりました。
実績データの詳細から切り離され、仕様を「先行確定」し、開発を「並行」に進められるようになりました。  
[click] 最も重要な成果は、ビジネス上、最も重要なコアとなる部分を、システムの中で「最も安定した位置に置くことができた」ことです。
-->
...
<!--
1つ目。  
以前は、関心事も混在し絡み合っている為に、幅広く影響を見ないといけませんし、それぞれの改修内容を気にする必要がありました。  
[click] それが「責務が明確になり、関心事にのみ集中できる」様になりました。  
[click] 高品質なAPIスキーマさえ定義できれば、レセプトチームもプロダクトチームも、双方が外部を気にせず自分の開発に全力集中できます。
-->
...
[click] 外部の事情に左右されず、契約、つまりインターフェースに基づいた厳密なテストが可能になり、Rezept as a Serviceが「単体のみで独立してテストができる」ようになりました。
-->
...
<!--
3つ目。  
以前は、実績データの仕様変更や、算定に必要なパラメータの仕様変更があった場合に密に連携が必要でした。又、双方の仕様が決まるまで最終的な連携ができませんでした。  
[click] それが「コミュニケーションコストが最低限」で済む様になりました。　 
インターフェースが決まれば、以降の開発フローでは基本的に一方通行のコミュニケーションでOKになります。    
[click] インターフェースとドキュメントが整備されたことで、他チームとの不要な調整が激減し、Rezept as a Serviceはセルフサービス型のプラットフォーム・サービスとして開発プロセスに組み込むことができました。
-->
...
<!--
4つ目。  
以前は、スケジュールの後半でバタバタテストを行うことが多かったです。又、手戻りが発生した場合の絶望感がすごかったです。  
[click] それが「システム全体の開発アジリティが向上」させることができました。  
 不安定だったレセプト業務が安定し、先行して開発・テストできるようになりました。　 
[click] 各プロダクトチームも並行開発が可能になり、プロジェクト全体の開発サイクルが大幅に短縮されました。
-->
...
<!--
それは、依存性逆転の前に、まず「関心事の分離」が必要だったからです。  
[click] 当時のシステムは、認知サイズが大きすぎました。  
[click] 境界が不明確で、どこで切るべきかわかりませんでした。  
[click] 複数の関心事が混在していたため、もし最初からインターフェースを切ろうとしていたら、複雑になりすぎていたでしょう。  
[click] まず関心事を分離して認知サイズを削減する、これが現実的なアプローチでした。
-->
...
<!--
初期フェーズからどんどん機能が追加されていき [click]「機能X」は「大きな泥団子」状態になっていきます。  
いっぱい関心ごとが重なり、結合度も高く、責任の境界も曖昧になることでしょう。  
この状態のシステムは非常に理解するのが大変です。
-->
...
<!--
更に移譲することでCoreを小さくするというのも大事です。  
[click] 先程紹介した「法改正の影響がない周辺の関心事を外に出す」という取り組みがこれに当たります。　 
こうすることで、関心ごとが更に少なくなり、責務がより明確になります。  
この明確化する行為がドメインの蒸留というイメージにピッタリではないでしょうか？  
余分なものをどんどん排除していきます。
-->
...
<!--
最後にインターフェースで分離して「依存性逆転」になります。  
[click] 関心ごとの分離や、責務の移譲を行なったとしてもどうしても、この図にあるように重なりがどうしても残ってしまうケースがあります。  
それは処理やデータフローの問題で、システムとして影響を受けてしまう部分です。  
この部分に対しての解決方法としては、先ほど説明した依存性逆転の法則で乗り越えることができます。  
重なりの部分をインターフェースにして、そのインターフェースに依存させることで、関心ごとが跨る部分を綺麗に分離して、それぞれを疎結合にすることができます。
一番結合度が少なく、何者にも依存していないというのが、一番安定している状態になります。
-->
...
<!--
抽象度が高くて、理解しづらい部分で調べてもよくわかりません。  
ではどうやって、コアドメインを見つければ良いのでしょうか？
-->
...
<!--
それを説明する為に、デザイン思考のフレームワークを拝借したいと思います。  
ダブルダイヤモンドというものです。  
問題発見（探索と定義）と解決策の創出（開発と実現）の2つのフェーズを持ちます。
-->
...
<!--
コアドメインを見つける為には、この問題領域に向き合う必要があると考えています。  
...
最近のボスに「あなたは戦術ばかりで、戦略がないね」と言われてハッとしました。  
戦略に紐づかない戦術は「それはhowなんよ〜」と同義なんだろうなーという気づきでした。
-->
...
これはあくまで、レセプト業務を扱っている弊社ならではの問題となります。  
これは皆さんが携われている事業では、まったく違った問題があると思うので、ぜひ言語化して見て頂きたいと思います。  
ただ敢えて、言語化させてみると  
[click] こちらの4つがコアドメインの判断軸になると個人的に考えています。
-->
...
「算定ルールを分けて管理する」「レセプト業務の関心事を凝集する」「専門知識を特定チームに集中させる」ということを方針としても良いかと考えました。  
実際のリアーキテクチャを行なった当時は、ここまで明確な戦略は自分自身が理解できていませんでした。今回の登壇を機に言語化できてよかったと感じています。
-->
...
<!--
この戦略に基づき、「レセプト業務」こそが、「コアドメイン」としてご理解頂けると思います。  
戦略としてRezept as a Serviceを作る！というミッションはありましたが、 [click] これは単に思いつきとかではなく、システムの安定性とアジリティを確保する為の【戦略的な投資判断】であるとご理解頂けると思います。
-->
...
[click] 今回時間の都合で細かく説明できませんが、Rezept as a Serivceを実現する為のスキーマ駆動の品質を向上させる為の取り組みを行ってきました。  
runnによるAPIシナリオテストや、tagprによるバージョン管理・リリースフローの整備、eg-r2によるAPIインターフェースの品質向上になります。  
こちらも別途記事でまとめていますので、よかったらご参照ください。
-->
