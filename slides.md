...
まずPHPカンファレンス福岡10周年おめでとうございます！  
またこの様な素晴らしいタイミングで登壇することが出来て大変光栄です。
-->
...
2度のリアーキテクチャの末、「Rezept as a Service」の実現に至った道のりをお話しします。  
どうやってサービスを実現し、どんなメリットがあったのかという「戦術」面と、そもそも何故それが必要だと判断したのかという「戦略」面について、お話しします。  
[click] 昨年のアドカレが元ネタとなっています。  
 👉️
まず初めに私たちが扱っている「レセプト業務」がどれだけ複雑か、視覚的にお見せします。
-->
...
先ほど見て頂いた分厚い本に、このルールに関する内容がまとめられています。
-->
...
<!--
レセプト業務には、3年ごとに大規模な法改正があります。  
これが非常に厄介で、インターフェース仕様書等の情報公開から実装完了まで約3ヶ月しかありません。  
...
データの流れと依存の関係が結びついてしまう

<img src="/rezept-data-flow.svg" />

<Arrow v-click="1" x1="70" y1="150" x2="70" y2="420" width="3" color="blue" />

<Arrow v-click="2" x1="480" y1="420" x2="480" y2="150" width="3" color="purple"  />

<!--
さらに、レセプト業務特有の構造上の問題があります。  
「レセプト業務」は、その計算根拠となる上流の「各種実績データ（体制状況、サービス利用等）」に依存する構造となっています。データフローの方向がそのままシステムの依存関係に直結しており、上流データが確定しなければ下流の請求処理（レセプト業務）が進められない「直列的なデータ依存構造」がありました。  
[click] データは上流で発生して、下流に流れるフローの向きとなります。  
[click] 依存はそのデータを参照する形となり、下流が上流に依存する向きとなります。
-->
...
[click] 結果として、レセプト業務が、上流のデータ構造の変更によって振り回されて、不安定化しやすい構造になっていました。
-->
...
依存している機能を開発しているチームと密に連携を行う必要があります。    
[click] また実績データの仕様の確定を待っていられない、時間的な制約もあります。
-->
...
<!--
法改正は、施行日が国によって定められており、加えて毎回内容が異なるため、対応はなかなかのハードワークです。
-->
...
SOLID原則の一つ、責務分離です。  
「機能や関心ごとをしっかり分け、一つのモジュールに一つの役割だけを持たせましょう」というやつです。
-->
...
<!--
具体的には2つのことを行いました。
1つは、法改正の対応で旧制度と新制度のロジックが混ざらないよう、時系列で関心ごとの分断をしました。  
...
[click] そして、保守性の向上です。役割を終えた旧制度のロジックを安全に削除でき、システムが青天井に複雑になることを防ぎます。  
[click] モジュールとして独立させやすくなりました  
責務が明確になって、サービスとして切り出しやすくなりました。  
また理解容易性も上がり、年々複雑化する法制度に対して、認知負荷が倍々に上昇しなくなりました。
-->
...
関心ごとの分離により、1 つのサービスの安定性は大きく改善され、法改正を乗り切った！  
（ここドヤりポイントです！よかったら拍手願います！🙇）
...
<div flex justify-center items-center>
  ...
  >
    ...
    <p text-2xl font-bold mt-6>
      ...
      <span text-red-300>より高度な構造化が必要!</span>
    </p>
  </div>
</div>

<!--
レセプト業務の専門性が高い現状で、個別最適を進めると、担当者依存による再現性の低下と、法改正時の対応スピードの限界が見えてきました。  
全社的なルール統一の困難さや専門エンジニアの不足から、このままでは次の法改正に対応するのは難しいと判断し、より高度な構造化が必要だと考えました
-->
...
<!--
こちらがRezept as a Serviceと各プロダクトとの連携をイメージした図です。   

レセプト業務、つまり算定ロジックを「上位モジュール」として位置づけました。
そして、「APIインターフェース」という「契約定義」を定めます。
各プロダクト、つまり下位モジュールは、この「契約」に従って実装します。
最も重要な点として、上位モジュールである「算定ロジック」もまた、自らが定義した「APIインターフェース」にのみ依存します。
これにより、依存関係が逆転します。

こうすることで、算定ロジックは実績データの詳細実装から隔離され、より安定し、変更に強い構造になります。
-->
...
Rezept as a Serviceを構築後に2024年の法改正も無事に乗り切ることが出来ました！
-->
...
<!--
ここまでで関心ごとの分離と依存性逆転について説明をしましたが、それによって具体的に何が「最高」だったのか？をまとめます。  
想定以上の効果が4つありました。
-->
...
「レセプト機能単体で、独立してテストができる」ようになりました。
-->
...
<!--
3つ目。  
以前は、実績データや、算定に必要なパラメータの仕様変更があった場合にチーム間で密に連携が必要でした。又、双方の仕様が決まるまで繰り返し調整が必要でした。    
[click] インターフェースを定めることで、以降の開発は基本的に一方通行のコミュニケーションで完結し、調整にかかるコストを削減することが出来ました。      
[click] インターフェースとドキュメントの整備により、他チームとの不要な調整が減り、開発スピードも上がりました。「Rezept as a Service」をセルフサービス型の請求プラットフォームとして位置づけることができました。
-->
...
<!--
4つ目。  
以前は、開発の終盤にテストが集中することが多くQAへの負担が大きかったです。又、見切り発車での並行開発で手戻りがコストも多くかかっていました。    
[click] それがインターフェス仕様を先に決めることで、並行開発することができるようになりました。  
テストの独立性と早期着手により、結合前に品質を確保できるようになりました。その結果、後工程への負荷を軽減することが出来ました。   
...
3. 関心ごとが混在している  
   → インターフェースが複雑（意図も不明瞭）になる。単一責務の原則に反し、リスクも大きくなる  
...
[click] まず、当時のシステムは、認知サイズが膨れ上がる懸念がありました。認知サイズが大きくなりすぎると把握できなくなります。　   
[click] 境界が不明確だと、依存関係も把握するのが難しいです。  
[click] 複数の関心ごとが混在していると、最初からインターフェースを切ろうとしたら、複雑になりすぎたり、意図が不明確なインターフェースになるでしょう。  
...
<div class="text-red-600 text-xl">関心ごとが混在（密結合）</div>
...
こうすることで、重なりも減り、責務も明確になっていき、認知サイズもお手頃サイズになります。
-->
...
# 「関心ごとの分離」や「依存性逆転の原則」は、ドメイン蒸留を行う為の「戦術（設計原則）」にすぎない

<!--
それは「関心ごとの分離」や「依存性逆転の原則」は、ドメイン蒸留を行う為の「戦術（設計原則）」にすぎないと考えるからです。
-->
...
# もっと様々なシチュエーションが考えられる
# その状況応じた適切な設計原則がまだまだ他にもある

<!--
今回「関心ごとの分離」や「依存性逆転の原則」を適用した事例を紹介しましたが、それが全てに当てはまるとは限りません。  
世の中のシステムはもっとバラエティ豊かで、複雑だと考えています。  
その中で、様々なシチュエーションに応じた戦術や設計原則があると考えています。
-->
...
# ドメイン蒸留（Domain Distillation）とは？


<blockquote>
システムの中で最も価値の高い「コアドメイン」を特定し、<br />
それ以外の「汎用サブドメイン」や「支援サブドメイン」と明確に分離する。
</blockquote>

<!--
ドメイン駆動設計（DDD）における「ドメイン蒸留」は、システムの中でビジネス的に最も重要で価値の高い部分（コアドメイン）を特定し、それ以外の部分とを明確に分離することです。  
目的は開発リソースとドメイン知識をコアドメインに集中させ、プロジェクト全体を成功に導くことにあります。  
つまりドメイン蒸留は、何に注力するかを決める「戦略」です。
-->
...
<!--
コアドメインは、この問題領域に存在しています。システムが「何を解決したいのか」というビジネス課題から見つける必要があります。  
解くべき問題を正しく認識することが重要で、
問題設定を間違うと、以降の課題解決における様々な判断を誤ることになります。
-->
...
戦略に紐づかない戦術は「具のないたこ焼き」や「ごぼう天が入っていない博多うどん」のようなものです。  
「それはhowなんよ〜」という大きな声が聞こえそうです。  
...
# <carbon-task-view /> 今回の問題の洗い出し（問題の探索）
...
<div class="p-6 bg-black/50 border-2 border-yellow-400 rounded-lg shadow-xl">
  ...
  <h3 class="text-2xl font-extrabold text-white mb-4">🔑 コアドメインの悲観的な判断軸</h3>
...
今回は、競争優位性の為の問題ではなく、事業リスクを回避する為の悲観的な問題設定となりました。  
こちらの問題は今回のケースで探索された内容であり、一概にコアドメインを定めるものではありません。  
...
# <carbon-strategy-play /> 解くべき問題に対する戦略（問題の定義）
...
>
  ...
  >
    <p class="text-2xl font-bold text-white tracking-wide">
      法改正対応のスピードと品質を、組織全体で安定的に維持・向上させる<br />
...
今回はレセプト業務をコアドメインとして、戦略に基づきRezept as a Serviceを構築する流れとなり [click] 専用チームも作るという投資判断も行われました。
-->
...
[click] 時間の都合で詳細を取り上げることができませんが、スキーマ駆動開発にも取り組んできました。  
このスキーマ駆動開発も戦略を実現する為の戦術になります。   
runn(ランエヌ)によるAPIシナリオテストや、tagpr(タグ・ピーアール)によるバージョン管理・リリースフローの整備、eg-r2によるAPIインターフェースの品質向上などはBaaSを支える戦術です。    
...
ここでいうサブドメインはドメイン蒸留の文脈で、今回の事例ではプロダクトチームとして説明してきました。
-->
