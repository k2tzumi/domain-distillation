...
[click] 結果として、レセプト業務が、上流のデータ構造の変更に対して影響を受けてしまい、不安定化しやすい構造になっていました。
-->
...
<!--
法改正は、施行日が国によって定められており、加えて毎回内容が異なるため、対応に多大な労力を強いられます。
-->
...
また理解容易性も上がり、認知しやすいお手頃サイズにもなりました。
-->
...
<!--
こちらがRezept as a Serviceの概念図になります。

レセプト業務、つまり算定ロジックを「上位モジュール」として配置します。
...
<!--
今回紹介した事例では偶々「関心ごとの分離」や「依存性逆転の原則」が適用できる状況があっただけです。  
世の中のシステムはもっとバラエティ豊かで、複雑だと考えています。  
その中で、様々なシチュエーションに応じた設計原則があると考えています。
-->
...
<!--
ドメイン駆動設計（DDD）における「ドメイン蒸留」の主な目的は、システムの中でビジネス的に最も重要で価値の高い部分（コアドメイン）を特定し、それ以外の汎用的な部分（汎用サブドメイン）と明確に分離することです。　　
目的は開発リソースと知恵をコアドメインに集中させ、プロジェクト全体を成功に導くことになります。  
...
runnによるAPIシナリオテストや、tagprによるバージョン管理・リリースフローの整備、eg-r2によるAPIインターフェースの品質向上などはBaaSを支える戦術です。    
